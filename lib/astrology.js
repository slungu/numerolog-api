...
/**
 * Calculează tranzitele actuale — poziția planetelor rapide AZI.
 * Folosit pentru context astrologic în mesajele zilnice.
 *
 * Input: data UTC (an, luna, zi, ora, minut). Pentru context zilnic, ora
 * 12:00 UTC e suficientă (Luna se mișcă ~13°/zi, deci 1° per oră — tolerabil).
 *
 * Output: poziția în semn + grade pentru Soare, Lună, Mercur, Venus, Marte.
 */
export function calculateTransits({ year, month, day, hour = 12, minute = 0 }) {
  const utDate = new Date(Date.UTC(year, month - 1, day, hour, minute, 0));
  ...
  return {
    ...
    metadata: {
      ephemeris: 'astronomy-engine',
      note: 'Poziții la 12:00 UTC pentru ziua dată',
    },
  };
}
